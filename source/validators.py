...
def validate_warc_gz_archives_directory(warc_gz_archives_directory: str):
    ...
    if not os.path.exists(warc_gz_archives_directory):
        log_error(f"Directory containing the warc.gz archives to search does not exist: {warc_gz_archives_directory}")
        sys.exit()
...
def validate_search_regex_definitions_directory(search_regex_definitions_directory: str):
    ...
    if not os.path.exists(search_regex_definitions_directory):
        log_error(f"Directory containing the regex definition .txt files to search with does not exist: {search_regex_definitions_directory}")
        sys.exit()

    if not glob.glob(search_regex_definitions_directory + '/*.txt'):
        log_error(f"Directory that should contain the regex definition .txt files to search with does not contain any: {search_regex_definitions_directory}")
        sys.exit()
...
def validate_results_output_directory(results_output_directory: str):
    ...
    if not os.path.exists(results_output_directory):
        log_error(f"Directory to output the search results to does not exist: {results_output_directory}")
        sys.exit()
